...
The manuscript and every code docstring are in English. This README exists in both
Korean ([`README.md`](README.md)) and English (this file); the Korean one is the default
because the author is Korean and it was written first. Several of the research records
are in Korean only, because they were written as working documents during the study
...
The path from that question to this repository ran through a long conversation with
ChatGPT and, for the implementation, with Claude Opus through Kiro. The transcripts are
not published. They contain a third party's private correspondence, which is the subject
of the next section, and a link that would expose it is not made more acceptable by being
a link.
